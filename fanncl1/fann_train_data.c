...
float fann_train_epoch_quickprop(struct fann *ann, struct fann_train_data *data)
{
	...
	for(i = 0; i < data->num_data; i++)
	{
		fann_run(ann, data->input[i], NULL);
		fann_compute_MSE(ann, data->output[i], NULL);
		fann_backpropagate_MSE(ann, NULL);
		fann_update_slopes_batch(ann, ann->first_layer + 1, ann->last_layer - 1, NULL);
	}
...
void sigfunc(int sig) {
    clReleaseCommandQueue(fptsclglob->hardware.queue);
    clReleaseContext(fptsclglob->hardware.context);
