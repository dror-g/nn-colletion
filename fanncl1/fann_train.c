...
#include "fann.h"
	...
	/*int knl_bmse = 0;
	sclSoft software_bmse, software_bmse2;
	sclHard hardware_bmse;
	int found;
	fann_type *error_pl_cl;
	cl_mem weightscl;
	cl_mem error_begincl;
	cl_mem consub1stconcl;*/
...
{
	fann_run(ann, input, NULL);

	fann_compute_MSE(ann, desired_output, NULL);

	fann_backpropagate_MSE(ann, NULL);
											...
											fann_type * desired_output)
{
	...
	fann_type *output_begin = fann_run(ann, input, NULL);
	fann_type *output_it;
...
FANN_EXTERNAL float FANN_API fann_get_MSEcl(struct fann *ann, struct fpts_cl *fptscl)
{
	unsigned int i;
	cl_int err;
	fann_type MSE_value = 0.0;
	if(ann->num_MSE)
	{
		clFinish(fptscl->hardware.queue); //BUUG
		fann_type *MSE_values = calloc(sizeof(fann_type), ann->num_output); //BUUG
		sclRead(fptscl->hardware, sizeof(fann_type)*ann->num_output, fptscl->MSE_values, MSE_values); //BUUG
		clFinish(fptscl->hardware.queue);
		for(i=0; i<ann->num_output; i++) MSE_value+=MSE_values[i];//fptscl->MSE_valuesm[i];
		float result = MSE_value / (float) ann->num_MSE;
		//clEnqueueUnmapMemObject(fptscl->hardware.queue, fptscl->MSE_values, MSE_values, 0, NULL, NULL);
		//free(MSE_values);
		//printf("fann_get_MSEcl = %f\n", result);
		//exit(EXIT_SUCCESS); //Because of freeze.
		return result;
	}
	else
	{
 ...
 */
void fann_compute_MSE(struct fann *ann, fann_type * desired_output, struct fpts_cl *fptscl)
{
	...
	struct _cl_buffer_region erritregcl;
	//fann_type  *error_begintst = calloc(sizeof(fann_type), ann->total_connections);
	fann_type tstf[128], MSE_value = 0.0;
	int tef, flag = 0;
	size_t truesize, global_size[2], local_size[2], offset[2], gs, ls = LOCALSIZE;
	cl_int err;
...
#endif
		//setclsizes(global_size, local_size, ann->total_neurons);
		/* Commented for fusion
		gs = ceil(ann->total_neurons/(float)ls)*ls;
	...
	else
	{
		...
		//clEnqueueWriteBuffer( fptscl->hardware.queue, fptscl->error_begincl, CL_TRUE, 0, ann->total_neurons*sizeof(fann_type), ann->train_errors, 0, NULL, NULL ); //because of BUUGG
		//sclWrite(fptscl->hardware, ann->total_neurons*sizeof(fann_type), fptscl->error_begincl, ann->train_errors); //because of BUUGG
		fann_type val = 0.0;
		//setclsizes(global_size, local_size, ann->total_neurons);
		/* Commented for fusion
		gs = ceil(ann->total_neurons/(float)ls)*ls;
	...
	truesize = last_layer_end - last_layer_begin;
	printf("truesize == %d\n", truesize);
	//setclsizes(global_size, local_size, truesize);
	gs = truesize;//ceil(truesize/(float)ls)*ls;
	unsigned int i;
#ifdef DEBUGCLW
	fptscwrite(ann, fptscl); //For debug.1!!	
#endif
	//printf("first[ann->MSE_value = %f, ann->num_MSE = %u]: ", ann->MSE_value, ann->num_MSE);
    /* Commented for fusion
    offset[0] = fptscl->alloutput_offset; offset[1] = 0;
    err = clEnqueueNDRangeKernel(fptscl->hardware.queue, fptscl->fl_comp_mse.kernel, 1, offset, &gs, NULL, 0, NULL, NULL );
	if ( err != CL_SUCCESS ) {
		printf( "\ninCMSE clEnqueue Error run: " );
		sclPrintErrorFlags( err );
	}*/ 
    fptscl->alloutput_offset+=ann->num_output;
#ifndef DEBUGCL
	clFlush(fptscl->hardware.queue);
	ann->num_MSE+=ann->num_output; //Instead of "++" below.
#endif
#ifdef DEBUGCL
	error_it = error_begin + (last_layer_begin - first_neuron); //Here we'll use clCreateSubBuffer
	for(; last_layer_begin != last_layer_end; last_layer_begin++)
	{
		neuron_value = last_layer_begin->value;
		neuron_diff = *desired_output - neuron_value;
		//*error_it = *desired_output; //For testing!!!11
		...
		*error_it = neuron_diff * fann_activation_derived(last_layer_begin->activation_function,\
	...
	//flushwaitandrelease(fptscl);
	testclvsc(fptscl, ann->train_errors, fptscl->error_begincl, ann->total_neurons, "cMSE");
	//fann_type *MSE_values = calloc(sizeof(fann_type), ann->num_output);
    //sclRead(fptscl->hardware, sizeof(fann_type)*ann->num_output, fptscl->MSE_values, MSE_values);
	for(i=0; i<ann->num_output;i++) MSE_value += fptscl->MSE_valuesm[i];//fptscl->MSE_valuesm[i];
	//free(MSE_values);
	if(MSE_value!=ann->MSE_value) printf("Cmse: ann->MSE_value = %f, *MSE_value = %f, numl = %d\n", ann->MSE_value, MSE_value, ann->num_output);
#endif
#ifndef DEBUGCL
	//clFinish(fptscl->hardware.queue);
#endif	
...
*/
void fann_backpropagate_MSE(struct fann *ann, struct fpts_cl *fptscl)
{
	//mtrace();
	fann_type tmp_error;
	unsigned int i, *connectionsu, flag = 0, j = 0, truesize, npl = 0, errbase, testcount;
	struct fann_layer *layer_it;
	struct fann_neuron test_neuron, *neuron_it, *last_neuron;
	...
	fann_type *outpl = calloc(ann->total_connections, sizeof(fann_type));
	int tstint = 0;
	cl_int err;
	size_t global_size[2], local_size[2], offset[2], gs, ls, lastlconns=0;
	testcount = 0;
	//unsigned int *tsterrs = calloc(sizeof(unsigned int), ann->total_connections);
	/* go through all the layers, from last to first.
	 * And propagate the error backwards */

#ifdef ALL_IN1_TEST
	offset[1] = 1;
	//if(fptscl->mc > UC) gs = UC;
	//else 
	gs = 1280;//256*(((fptscl->mc>>3) + 255)/256); //!!!!
	ls = 256;
	printf("bpmseall gs==%d\n", gs);
	err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_bmseall.kernel, 1, 0, &gs, 0, 0, NULL, NULL);
	if ( err != CL_SUCCESS ) {
		printf( "\ninBpMSE clEnqueue Error run1: " );
		sclPrintErrorFlags( err );
	}
	clFlush(fptscl->hardware.queue);
#endif
	for(layer_it = last_layer - 1; layer_it > second_layer; --layer_it)
	{
		...
		//setclsizes(global_size, local_size, layer_it->countonn);
#ifndef ALL_IN1_TEST
		gs = layer_it->countonn;//ceil(layer_it->countonn/(float)ls)*ls;
		err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_bmser.kernel, 1, &lastlconns, &gs, NULL, 0, NULL, NULL);
		if ( err != CL_SUCCESS ) {
			printf( "\ninBpMSE clEnqueue Error run1: " );
			sclPrintErrorFlags( err );
		}
#endif		
		printf("[gs == %lu; lastlconns == %lu]", layer_it->countonn, lastlconns);
		lastlconns+=layer_it->countonn;
#ifdef DEBUGCL
		/*for(j = lastlconns; j < lastlconns+layer_it->countonn; j++)
		{
			i = ann->alllconns[j];
			error_begin[ann->consub1stcon[i]]+=error_begin[ann->tmp_errors[j]]*ann->weights[i];
			printf("M.error_begin[%d] += error_begin[%d](%f)*%f\n", ann->consub1stcon[i], ann->tmp_errors[j], error_begin[ann->tmp_errors[j]], ann->weights[i]);//DEBUG
		}
		lastlconns = j;*/
		//printf("ann->tmp_errors[%d]==%d; neuron_it - first_neuron == %lu; ", j, ann->tmp_errors[j], layer_it->first_neuron - first_neuron);
		...
		if(ann->connection_rate >= 1)
		{
			...
			for(neuron_it = layer_it->first_neuron; neuron_it != last_neuron; neuron_it++)
			{
				...
				for(i = neuron_it->last_con - neuron_it->first_con; i--;)
				{
					 ...
					 * printf("weights[%d] = %f\n", i, weights[i]); */
					//if(i<83) printf("%d, %f\n", i, weights[i]);
				//	j = neuron_it->first_con + i;//TEST TEST
					//outpl[fptscl->revrevconnsh[j]] = (neuron_it - first_neuron)*0.01;// * weights[i];//(neuron_it - layer_it->first_neuron)*0.01;//weights[i];//weights[i];//tmp_error;//weights[i];//TEST TEST
					error_prev_layer[i] += tmp_error * weights[i]; //
					//if(layer_it == last_layer - 1) printf("%u) %f, tmp_error == %f\n", neuron_it->first_con+i, weights[i], tmp_error);
				}
			}
		}
		else
		...
		//printf("ann->tmp_errors[%d]==%d; neuron_it - first_neuron == %lu;\n", j-1, ann->tmp_errors[j-1], last_neuron - 1 - first_neuron);
		testcount++;
...
#ifndef ALL_IN1_TEST
		truesize = (layer_it - 1)->last_neuron - (layer_it - 1)->first_neuron;
		gs = truesize;//ceil(truesize/(float)ls)*ls; //layer_it->countonn
		errbase = ((layer_it - 1)->first_neuron - first_neuron);
		offset[0] = errbase; //DEBUG
		err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_bmse2.kernel, 1, offset, &gs, NULL, 0, NULL, NULL);
		if ( err != CL_SUCCESS ) {
			printf( "\ninBpMSE clEnqueue Error run2: " );
			sclPrintErrorFlags( err );
		}
		clFlush(fptscl->hardware.queue);
#endif
...
#ifdef DEBUGCL
	testclvsc(fptscl, ann->train_errors, fptscl->error_begincl, ann->total_neurons, "bpMSE");
	//testclvsc(fptscl, outpl, fptscl->outpl, ann->total_connections, "bpMSE_o");
	free(outpl);
	//exit (0);
#endif
#ifndef DEBUGCL
...
void fann_update_slopes_batch(struct fann *ann, struct fann_layer *layer_begin,
							  struct fann_layer *layer_end, struct fpts_cl *fptscl)
{
	struct fann_neuron *neuron_it, *last_neuron, *prev_neurons, **connections;
	fann_type tmp_error;
	struct fann_layer *layer_begins;
	unsigned int i,j, num_connections, flag = 0, truesize, *truesizem;
	/* store some variabels local for fast access */
	struct fann_neuron *first_neuron = ann->first_layer->first_neuron;
	fann_type *error_begin = ann->train_errors;
	fann_type *slope_begin, *neuron_slope;
	size_t global_size[2], local_size[2], offset[2], gs, ls = LOCALSIZE;
	cl_int err, returnparam=CL_RUNNING, returnparamold;
...
#endif
	prev_neurons = first_neuron;
	//printf("r<1: base = %u, global_size[0] = %zu\n", base, global_size[0]);
	//printf("bmse launch \n");
	fann_type tstf;
	int tstint;
#ifdef DEBUGCLW
	fptscwrite(ann, fptscl); //For debug.1!!
#endif
	layer_begins = layer_begin;
	//setclsizes(global_size, local_size, ann->tc_in_layers);
	gs = 1024;//ceil(ann->tc_in_layers/(float)ls)*ls;
	//printf("upslr gs = %lu, ls = %lu\n", gs, ls);
	//offset[0] = layer_begins->first_neuron - first_neuron; offset[1]=0;
	err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_upslr.kernel, 1, NULL, &gs, NULL, 0, NULL, NULL );
	if ( err != CL_SUCCESS ) {
			printf( "\ninslopes clEnqueue Error: " );
			sclPrintErrorFlags( err );
	}
	/*for(; layer_begins <= layer_end; layer_begins++)
	{
		truesize = layer_begins->last_neuron-layer_begins->first_neuron; ////!= last_neuron;
		setclsizes(global_size, local_size, truesize);
		offset[0] = layer_begins->first_neuron - first_neuron; offset[1]=0;
		err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_upsl.kernel, 1, offset, global_size, NULL, 0, NULL, NULL );
		if ( err != CL_SUCCESS ) {
			printf( "\ninslopes clEnqueue Error: " );
			sclPrintErrorFlags( err );
		}
...
#ifdef DEBUGCL
	printf("r<1: offset[0] = %zu, global_size[0] = %zu\n", offset[0], global_size[0]);
	for(; layer_begin <= layer_end; layer_begin++)
	{
		...
		for(neuron_it = layer_begin->first_neuron; neuron_it != last_neuron; neuron_it++)
		{
			...
			for(i = 0; i != num_connections; i++)
			{
				...
				//if(neuron_it->first_con+i==8165) printf("CPU: idx==%d, pnr==%d\n", neuron_it->first_con+i, neuron_it - first_neuron);
			}
		}
	}
	/*printf("ann->tc_in_layers==%lu, ann->total_connections==%u\n", ann->tc_in_layers, ann->total_connections);
	for(j=0; j<ann->tc_in_layers; j++){
		i = ann->alllconns[j];
		//printf("i%u|",i);
		slope_begin[i] += error_begin[ann->tmp_errors[j]] * ann->connections[i]->value;
	}
	printf("\n");*/
	
	//flushwaitandrelease(fptscl);
	testclvsc(fptscl, ann->train_slopes, fptscl->train_slopescl, ann->total_connections, "upSlops");
#endif
#ifndef DEBUGCL
	//clFinish(fptscl->hardware.queue);
#endif	
 ...
 */
void fann_clear_train_arrays(struct fann *ann, struct fpts_cl *fptscl)
{
	unsigned int i, err;
	...
	//sclWrite(fptscl->hardware, ann->total_connections_allocated*sizeof(fann_type), fptscl->train_slopescl, ann->train_slopes);
	clearclarray(&fptscl->train_slopescl, ann->total_connections_allocated, fptscl);
	...
	if(ann->training_algorithm == FANN_TRAIN_RPROP)
	{
		...
		sclWrite(fptscl->hardware, ann->total_connections_allocated*sizeof(fann_type), fptscl->prev_steps, ann->prev_steps);
	}
	else
	{
		memset(ann->prev_steps, 0, (ann->total_connections_allocated) * sizeof(fann_type));
		clearclarray(&fptscl->prev_steps, ann->total_connections_allocated, fptscl);
	}
	/* if no room allocated for the variabels, allocate it now */
	...
	clearclarray(&fptscl->prev_train_slopes, ann->total_connections_allocated, fptscl);
	clearclarray(&fptscl->outpl, ann->total_connections, fptscl);
}
...
*/
void fann_update_weights_irpropm(struct fann *ann, unsigned int first_weight, unsigned int past_end, struct fpts_cl *fptscl)
{
	fann_type *train_slopes = ann->train_slopes;
	fann_type *weights = ann->weights;
	fann_type *prev_steps = ann->prev_steps;
	...
	fann_type prev_step, slope, prev_slope, next_step, same_sign, zero = 0.0;
	size_t global_size[2], local_size[2], offset[2], gs, ls = LOCALSIZE;
	...
	cl_int err;
	unsigned int i = first_weight, flag = 0, truesize = past_end-first_weight;
	//printf("first_weight = %u, past_end = %u\n", first_weight, past_end);
#ifdef DEBUGCLW
	fptscwrite(ann, fptscl); //For debug.1!!
#endif
	//setclsizes(global_size, local_size, truesize);
	gs = truesize;
	offset[0]=first_weight; offset[1]=1;
#ifdef DEBUGCL
	printf("global_size[0]=%lu, ann->total_connections=%d, past_end=%d, offset=%lu\n", gs, ann->total_connections, past_end, offset[0]);
#endif
	err = clEnqueueNDRangeKernel( fptscl->hardware.queue, fptscl->software_uwirpr.kernel, 1, offset, &gs, NULL, 0, NULL, NULL );
	if ( err != CL_SUCCESS ) {
		printf( "\ninUwIrpr clEnqueue Error run: " );
		sclPrintErrorFlags( err );
	}
	err = clEnqueueFillBuffer(fptscl->hardware.queue, fptscl->in_off, &zero, sizeof(fann_type), 0, sizeof(fann_type), 0, NULL, NULL );
	err = clEnqueueFillBuffer(fptscl->hardware.queue, fptscl->out_off, &zero, sizeof(fann_type), 0, sizeof(fann_type), 0, NULL, NULL );
	if ( err != CL_SUCCESS ) {
		printf( "\ntestclvsc clEnqueueFillBuffer Error - 2: " );
		sclPrintErrorFlags( err );
	}
	err = clFlush(fptscl->hardware.queue);
	if ( err != CL_SUCCESS ) {
		printf( "\ntestclvsc clFlush Error: " );
		sclPrintErrorFlags( err );
	}
#ifdef DEBUGCL
	for(; i != past_end; i++)
	{
		...
		//if(same_sign < 0.0001 && same_sign>0) printf("%u)CPU: same_sign==%f\n", i, same_sign);
		if(slope < 0)
		{
			weights[i] -= next_step;
			if(weights[i] < -1500)
	...
	testclvsc(fptscl, ann->train_slopes, fptscl->train_slopescl, ann->total_connections, "upWts.sl");
	testclvsc(fptscl, ann->prev_train_slopes, fptscl->prev_train_slopes, ann->total_connections, "upWts.psl");
	testclvsc(fptscl, ann->prev_steps, fptscl->prev_steps, ann->total_connections, "upWts.pst");
	testclvsc(fptscl, ann->weights, fptscl->weightscl, ann->total_connections, "upWts.wts");
	//fptscwrite(ann, fptscl); //For debug.1!!
#endif
#ifndef DEBUGCL
	//clFinish(fptscl->hardware.queue);
