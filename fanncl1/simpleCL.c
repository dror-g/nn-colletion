...
void _sclBuildProgram_opt( cl_program program, cl_device_id devices, const char* pName, const char *options )
{
#ifdef DEBUG
	cl_int err;
	char build_c[4096];
	...
	*found = 1;
	...
	for ( i = 0; i < nTotalDevs; ++i ) {
		...
		if ( i == nDevice ) {
			...
			err  = clGetDeviceInfo( devices[i], CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(sizes), (void*)sizes, &returned_size );
			for(j = 0; j < returned_size/sizeof(size_t); j++)
				printf("MAX_SIZE %d = %lu; ", j+1, sizes[j]);
			printf(" ]\n");
		}
	}
	if ( *found ) {
		/* Create context ########################################################### */
		cl_context_properties properties[] = {CL_CONTEXT_PLATFORM, (cl_context_properties)(hardware.platform), 0};
		hardware.device = devices[nDevice];
		hardware.context =  clCreateContextFromType(properties, CL_DEVICE_TYPE_GPU, NULL, NULL, &err);//hardware.context = clCreateContext( 0, 1, &hardware.device, NULL, NULL, &err );
		if ( err != CL_SUCCESS) {
			printf( "\nError 3" );
			sclPrintErrorFlags( err ); }
		/* ########################################################### */
		...
		hardware.queue = clCreateCommandQueue( hardware.context, hardware.device,  NULL, &err );//CL_QUEUE_PROFILING_ENABLE|CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE
		if ( err != CL_SUCCESS ) {
			printf( "\nError 3.1" );
			sclPrintErrorFlags( err ); }
		/* ########################################################### */	
...
sclHard sclGetCPUHardware( int nDevice, int* found ) {
	...
	*found = 1;
	...
	platformName = (char *)malloc( sizeof(char) * 64 );
	devices = (cl_device_id *)malloc( sizeof(cl_device_id) * 8 );
	/*Get platform info ###################################################### */
		...
		*found = 0;
		...
		if ( nCPUplatforms == 0 ) {
			...
			*found = 0;
			...
			err = clGetPlatformInfo ( CPUplatforms[vnd], CL_PLATFORM_VENDOR, (size_t)30, (void *)platformName, NULL);
			if ( err != CL_SUCCESS ) {
				printf( "\nError clGetPlatformInfo" );
				sclPrintErrorFlags( err ); }
			printf( "\nMore than one OpenCL platform with enabled CPU's.\nUsing: %s", platformName );
			hardware.platform = CPUplatforms[vnd];
			hardware.device = devices[nDevice];
		}
...
//#endif
	...
	if ( *found ) {
		...
		cl_context_properties properties[] = {CL_CONTEXT_PLATFORM, (cl_context_properties)(hardware.platform), 0};
		hardware.device = devices[nDevice];
		hardware.context =  clCreateContextFromType(properties, CL_DEVICE_TYPE_CPU, NULL, NULL, &err);//clCreateContext(NULL, 1, &hardware.device, NULL, NULL, &err );
		if ( err != CL_SUCCESS) {
			printf( "\nError 3" );
			sclPrintErrorFlags( err ); }
		/* ########################################################### */
		...
		/* Create command queue ########################################################### */	
		hardware.queue = clCreateCommandQueue( hardware.context, hardware.device,  NULL, &err ); //CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE//CL_QUEUE_PROFILING_ENABLE
		if ( err != CL_SUCCESS ) {
			printf( "\nError 3.1" );
			sclPrintErrorFlags( err ); }
		/* ########################################################### */	
...
char* strrepl(const char *oldstr, const char *replstr, const char *str)
{
 // char str[] ="This is a simple string";
  char* newstr = calloc(strlen(str)+strlen(replstr), sizeof(char));
  char* currstr;
...
sclSoft sclGetCLSoftware_opt( char* path, char* name, sclHard hardware, const char *options,  const char *oldstr, const char *replstr){
	sclSoft software;
	/* Load program source
	 ########################################################### */
	char *oldsource = _sclLoadProgramSource( path );
	char *source;
	char filename[512];
	FILE *fh; 
	/* ########################################################### */
	if(oldstr){
		source = strrepl(oldstr, replstr, oldsource);
		//puts(source);
		/*FILE *f = fopen("/tmp/test_bpall.cl", "wb");
		//fprintf(f, "%s", source);
		fwrite(source, sizeof(char), strlen(source), f);
		fclose(f);*/
		//source = _sclLoadProgramSource( "/tmp/test_bpall.cl" );
	} else {
		source = oldsource;
	}
	sprintf(filename, "/tmp/_%s.cl", name);
	fh = fopen(filename, "w" );
	fwrite(source, sizeof(char), strlen(source), fh);
	fclose(fh);
	sprintf( software.kernelName, "%s", name);
	...
	/* Build the program (compile it)
   	 ############################################ */
   	_sclBuildProgram_opt( software.program, hardware.device, name, options );
   	/* ############################################ */
...
//#ifdef DEBUG
  //HeapLeakChecker heap_checker1("Manage Args");
  //{
//#endif
	va_start( argList, sizesValues );
	for( p = sizesValues; *p != '\0'; p++ ) {
...
//#ifdef DEBUG
	...
	//HeapLeakChecker heap_checker2("Launch & finish");
	//{
//#endif	
