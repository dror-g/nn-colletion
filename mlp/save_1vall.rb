...
#a = MLP.new(:hidden_layers => [1], :output_nodes => 1, :inputs => 12)
#a = MLP.new(:hidden_layers => [120], :output_nodes => 1, :inputs => 9)
a = MLP.new(:hidden_layers => [1], :output_nodes => 1, :inputs => 18)
#a = MLP.new(:hidden_layers => [7], :output_nodes => 1, :inputs => 15)
...
master = ccc.query("SELECT D from train group by D limit 10")
#master=[["#{v1}"],["#{v2}"]]
master.each do |m| 
...
begin
	...
	if m[0] == me[0]
 	  rs = con.query("SELECT X,Y,Z from train where D = '#{m[0]}' limit 60")
    rs.each do
...
#zu=[wak[i],wak[i+1],wak[i+2],wak[3]]

#(0..40).step(3) do |i|
#zu=[wak[i],wak[i+1],wak[i+2],wak[3],wak[4]]
...
#zu=[wak[i],wak[i+1],wak[i+2]]
(0..12).step(3) do |i|
zu=[wak[i],wak[i+1],wak[i+2],wak[i+3],wak[i+4],wak[i+5]]
zu=zu.flatten(1)
zu=zu.flatten(1)
...
File.open("nets/#{v1}", 'w') {|f| f.write(Marshal.dump(a)) }
  #puts "Error after iteration \t#{error}"# if e%20 == 0
#break
end
#puts "Test data"
